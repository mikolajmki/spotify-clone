    ...
    return (
        <Modal title="Add a song" description="Upload an mp3 file" isOpen={uploadModal.isOpen} onChange={onChange}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-y-4">
                ...
                <Input id="author" disabled={isLoading} placeholder="Song author" { ...form.register("author", { required: true }) }/>
                <div>
                    <div className="pb-1">
                        Select a song file
                    </div>
                    <Input id="song" type="file" disabled={isLoading} accept=".mp3" { ...form.register("song", { required: true }) }/>
                </div>
                <div>
                    <div className="pb-1">
                        Select an image
                    </div>
                    <Input id="image" type="file" disabled={isLoading} accept="image/*" { ...form.register("image", { required: true }) }/>
                </div>
